...
## 13. String Memory Management Pattern

### The Problem
During optimization efforts, replacing `std::string` with `const char*` can break async operations because:

1. `config.panelName.c_str()` returns a temporary pointer
2. `splashTargetPanel_ = panelName` stores the pointer (not the data)
3. By the time splash completes (~1500ms), the source string is deallocated
4. System tries to load panel with garbage data → crash
5. **Same issue affects restoration**: `restorationPanel = currentPanel` copies pointer, not data
