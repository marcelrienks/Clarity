...
std::unique_ptr<IPanel> UIFactory::createLockPanel(IGpioProvider *gpio, IDisplayProvider *display,
                                                   IStyleService *styleService)
{
    return std::make_unique<LockPanel>(gpio, display, styleService);
}

std::unique_ptr<IPanel> UIFactory::createSplashPanel(IGpioProvider *gpio, IDisplayProvider *display,
                                                     IStyleService *styleService)
{
    return std::make_unique<SplashPanel>(gpio, display, styleService);
}

std::unique_ptr<IPanel> UIFactory::createOemOilPanel(IGpioProvider *gpio, IDisplayProvider *display,
                                                     IStyleService *styleService)
{
    return std::make_unique<OemOilPanel>(gpio, display, styleService);
}

std::unique_ptr<IPanel> UIFactory::createErrorPanel(IGpioProvider *gpio, IDisplayProvider *display,
                                                    IStyleService *styleService)
{
    return std::make_unique<ErrorPanel>(gpio, display, styleService);
}

std::unique_ptr<IPanel> UIFactory::createConfigPanel(IGpioProvider *gpio, IDisplayProvider *display,
                                                     IStyleService *styleService)
{
