...
void InterruptManager::Init(IGpioProvider* gpioProvider)
{
    ...
    // Create and register handlers with GPIO provider
    if (gpioProvider) {
        ...
        auto triggerHandler = std::make_shared<TriggerHandler>(gpioProvider);
        auto actionHandler = std::make_shared<ActionHandler>(gpioProvider);
        
        if (polledHandler && queuedHandler && triggerHandler && actionHandler)
        {
            ...
            log_d("Registered all handlers: PolledHandler, QueuedHandler, TriggerHandler, ActionHandler");
        }
        else
        {
            log_e("Failed to create all handlers with GPIO provider");
            ErrorManager::Instance().ReportError(ErrorLevel::ERROR, "InterruptManager", 
                                               "Failed to create all interrupt handlers");
        }
    } else {
...
void InterruptManager::Process()
{
    ...
    // NEW EVALUATION TIMING MODEL:
    // Actions evaluate ALWAYS (every main loop)
    // Triggers evaluate only during IDLE
    
    // Phase 1: Process Actions (continuous evaluation)
    if (actionHandler_) {
    ...
    // Legacy Phase: Keep existing interrupt processing for backward compatibility
    // Phase 3: Evaluate legacy interrupts (check for state changes)
    EvaluateInterrupts();
    
    // Phase 4: Execute legacy interrupts that need action (only during idle)
    if (IsUIIdle()) {
        ExecuteInterrupts();
    }
...
// New Trigger/Action system methods
bool InterruptManager::RegisterTrigger(const Trigger& trigger) {
    log_v("RegisterTrigger() called for '%s'", trigger.id);
...
void InterruptManager::UnregisterTrigger(const char* id) {
    log_v("UnregisterTrigger() called for '%s'", id);
...
bool InterruptManager::RegisterAction(const Action& action) {
    log_v("RegisterAction() called for '%s'", action.id);
...
void InterruptManager::UnregisterAction(const char* id) {
    log_v("UnregisterAction() called for '%s'", id);
...
void InterruptManager::UpdatePanelFunctions(void (*shortPressFunc)(), void (*longPressFunc)()) {
    ...
    if (actionHandler_) {
        ...
        log_d("Updated panel functions in ActionHandler");
    } else {
