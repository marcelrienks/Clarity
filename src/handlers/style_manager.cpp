...
void StyleManager::init(Theme theme) {
    log_i("Initializing styles with theme: %d", theme);
...
void StyleManager::init_styles() {
    log_d("Initializing style objects");
...
void StyleManager::update_styles() {
    log_d("Updating styles with current theme colors");
    
    // Select the current theme colors
    ThemeColors* colors = (_current_theme == Theme::Dark) 
                        ? &_dark_theme_colors 
                        : &_light_theme_colors;
...
void StyleManager::switch_theme(Theme new_theme) {
    log_i("Switching theme from %d to %d", _current_theme, new_theme);
    
    if (_current_theme == new_theme) {
        log_d("Theme is already set to %d, no change needed", new_theme);
        return;
    }
...
void StyleManager::apply_theme_to_screen(lv_obj_t* screen) {
    log_d("Applying theme to screen");
